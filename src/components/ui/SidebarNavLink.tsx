...
export default function SidebarNavLink({ name, icon, path }: { name: string, icon: string, path: string }) {
    return <NavLink activeClass='text-blue-400' className={cn(buttonVariants({ type: 'ghost', className: 'w-fit font-medium xl:pr-5 max-xl:w-14 max-xl:h-14 max-xl:justify-center group transition-all data-[active=true]:font-semibold' }))} href={path}>
